...
export default (boardSize, mineNum) => {
    ...
    while(mineCount < mineNum){
        ...
        if(board[x][y].value === 0){            // Check this location has not been located a mine.
            ...
            if(x-1 >= 0 && y >= 0) board[x-1][y].value += 1;
            if(x-1 >= 0 && y+1 < boardSize) board[x-1][y+1].value += 1;
            if(x >= 0 && y-1 >= 0) board[x][y-1].value += 1;
            if(x >= 0 && y+1 < boardSize) board[x][y+1].value += 1;
            if(x+1 < boardSize && y-1 >= 0) board[x+1][y-1].value += 1;
            if(x+1 < boardSize && y >= 0) board[x+1][y].value += 1;
            if(x+1 < boardSize && y+1 < boardSize) board[x+1][y+1].value += 1;
            mineCount++;
        }
    }
